...
public class Day11_4_BoardApp {
	 ...
	 *  	// [m]게시물 클래스
	 *  		// 번호 = 인덱스 , 제목 , 패스워드 , 내용 , 작성자 , 작성일 , 조회수 , 댓글리스트
	 *  
	 *  	// [m]댓글 클래스
	 *  		// 내용 , 패스워드 , 작성자 , 작성일
	 *    
			...
			try { // 예외발생 : 사용자가 문자입력시 예외발생!!! -> catch 이동 
				...
				if( ch == 1 ) {
					...
					if( ch2 == 1 ) { }
						...
						scanner.nextLine();// [ 일반next 다음로 nextLine 사용하게되면 문제발생 [개행 취급] ]
						System.out.println(" 변경 제목 :"); 		  String title = scanner.nextLine();
						System.out.println(" 변경 내용 : ");		  String contents = scanner.nextLine();
						boolean result = Controller.update( index , password , title , contents ); // 수정할 인덱스번호,패스워드,수정할제목,수정할내용 =>  인수 
