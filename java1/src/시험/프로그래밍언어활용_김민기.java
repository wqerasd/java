...
public class 프로그래밍언어활용_김민기 { // c s

	public static void main(String[] args) { // m s
			
		// 배열 
		Student[] 학생배열 = new Student[100];
		...
		while( true ) {
			try {
				...
				for( int j = 0 ; j<학생배열.length; j++ ) {
					...
					if( 학생배열[j] != null) {
						System.out.printf("%d\t%s\t%d\t%d\t%d\t%d\t%.2f\t%d\n" , (j+1),학생배열[j].get이름(),학생배열[j].get국어(),
								학생배열[j].get영어(),학생배열[j].get수학(),학생배열[j].get총점(),학생배열[j].get평균(),Rank[j]);				
					}
				...
				if( ch==1 ) {   // 학생 등록 
					...
					Student member = new Student( 이름 , 국어 , 영어 , 수학 ,총점 , 평균);
					int i = 0;
					
					for( Student temp : 학생배열 ) {
						if( temp == null) {
							학생배열[i] = member; break;
						} i++;
					}
					...
					for(int r = 0; r<Rank.length; r++  ) {
						for(int t = 0; r<Rank.length; t++) {
							...
							else if(학생배열[r].get총점() > 학생배열[t].get총점()) {
								Rank[t]++;
							}
						}
					}
				...
				System.out.println(" 알림]] 알수없는 명령입니다.");
